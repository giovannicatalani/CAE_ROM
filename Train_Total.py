...
if train_model == True:
    ...
    for epoch in range(1000):
        tr_loss, tr_loss_lat, tr_loss_total = run_epoch(
            epoch, model, model_lat, train_loader, criterion, optimizer, optimizer_lat, weight_loss=weight_loss, device=device)
        valid_loss, valid_loss_lat, valid_loss_total = run_val(
            epoch, model, model_lat, valid_loader, criterion, weight_loss=weight_loss, device=device)
        ...
        if valid_loss < state['best_valid_loss']:
            ...
            state['epoch'] = epoch
            state['state_dict_AE'] = model.state_dict()
